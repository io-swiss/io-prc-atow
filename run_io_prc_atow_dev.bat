...
rem ----------------------------------------------------------------------------
rem
rem run_io_prc_atow_dev.bat: Process IO-PRC-ATOW tasks.
rem
rem ----------------------------------------------------------------------------
...
set IO_AERO_PRC_ATOW_LOG=run_io_prc_atow_dev_%IO_AERO_TASK%.log
...
> %IO_AERO_PRC_ATOW_LOG% 2>&1 (
    ...
    if /I ["%IO_AERO_TASK%"] EQU ["version"] (
        ...
        if ERRORLEVEL 1 (
            echo Processing of the script run_io_prc_atow_dev was aborted
            exit 1
        )
...
echo Processing of the script run_io_prc_atow_dev is aborted: unknown task='%IO_AERO_TASK%'
exit 1
